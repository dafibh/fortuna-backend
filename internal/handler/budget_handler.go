...
// BudgetMonthResponse represents the budget data for a month
...
func (h *BudgetHandler) GetAllocations(c echo.Context) error {
	...
	result, err := h.allocationService.GetAllocationsForMonth(workspaceID, year, month)
	if err != nil {
		log.Error().Err(err).Int32("workspace_id", workspaceID).Int("year", year).Int("month", month).Msg("Failed to get budget allocations")
		return NewInternalError(c, "Failed to get budget allocations")
	}
